...
/**
 * Scores every rule instead of returning the first array match.
 *
 * The old first-match logic classified "I design logos for fashion brands"
 * as `fashion`, purely because the fashion rule sat earlier in the array.
 * Counting matches lets the dominant signal win regardless of rule order.
 */
export function classifyBusiness(description: string): string {
  ...
  for (const [category, expression] of CATEGORY_RULES) {
    const global = new RegExp(expression.source, `${expression.flags.replace('g', '')}g`);
    const score = (normalized.match(global) ?? []).length;
    if (score > bestScore) {
      bestScore = score;
