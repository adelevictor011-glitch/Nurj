...
function App() {
  ...
  const pendingIntent = useRef<ScreenKey | null>(null);

  const syncStatus = useCallback(async (options: { redirect?: boolean } = {}) => {
    if (!user) return;
    try {
      const status = await api.status();

      // Carry anything the guest already told us into the new account instead
      // of throwing it away and making them redo the signal scan.
      const local = loadLocal<UserProfile>('nurj-profile-v2', DEFAULT_PROFILE);
      const carriedStage = status.profile.stage ?? local.stage;
      const merged: UserProfile = {
        ...status.profile,
        stage: carriedStage,
        business_description: status.profile.business_description ?? local.business_description,
        target_customer: status.profile.target_customer ?? local.target_customer,
        business_category: status.profile.business_category ?? local.business_category,
        onboarding_complete: status.profile.onboarding_complete || Boolean(carriedStage),
      };
      ...
      const carriedSomething =
        merged.stage !== status.profile.stage ||
        merged.onboarding_complete !== status.profile.onboarding_complete ||
        merged.business_description !== status.profile.business_description ||
        merged.target_customer !== status.profile.target_customer;

      if (carriedSomething && supabase) {
        const { error } = await supabase
          .from('profiles')
          .update({
            stage: merged.stage,
            business_description: merged.business_description,
            target_customer: merged.target_customer,
            business_category: merged.business_category,
            onboarding_complete: merged.onboarding_complete,
            updated_at: new Date().toISOString(),
          })
          .eq('id', status.profile.id);
        if (error) console.error('[nurj] could not persist carried guest context', error.message);
      }

      // Only steer the user on the first sync. Background refreshes must never
      // yank someone out of the screen they are working in.
      if (options.redirect ?? !initialSyncDone.current) {
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      void syncStatus({ redirect: false });
    };
  ...
  const runVerification = useCallback(
    async (reference: string) => {
      ...
      // Paystack settlement and our own webhook can both lag the redirect.
      // Retry with backoff instead of failing the customer on attempt one.
  ...
  useEffect(() => {
    ...
    // A reference stored before redirect survives a lost callback URL, a
    // different browser, or a phone that ran out of data mid-checkout.
    const stored = loadLocal<string | null>('nurj-pending-reference', null);
    const reference = fromUrl ?? stored;
    if (!reference) return;
    setPendingReference(reference);
    if (!user) return;
    void runVerification(reference);
  }, [runVerification, user]);
  ...
  async function beginPayment(plan: 'builder' | 'operator') {
    if (!user) {
      ...
      pendingIntent.current = 'upgrade';
      notify('Sign in to activate your plan.');
      await signIn();
      return;
    }
    setPaymentBusy(plan);
    try {
      ...
      localStorage.setItem('nurj-pending-reference', JSON.stringify(checkout.reference));
  ...
  return (
    <div className="app-root">
      ...
      {pendingReference && verifyState !== 'idle' && (
        <PaymentBanner
          state={verifyState}
          onRetry={() => void runVerification(pendingReference)}
          onDismiss={() => {
            localStorage.removeItem('nurj-pending-reference');
            setPendingReference(null);
            setVerifyState('idle');
...
function Dashboard({
  ...
  const stageKey = profile.stage ?? 'launch';

  // action_progress existed in the schema, fully secured, and was never used.
  // Progress now survives a new phone or a cleared browser.
  useEffect(() => {
    ...
    void supabase
      ...
      .then(({ data, error }) => {
        ...
        const indexes = data
          .filter((row) => row.completed && String(row.action_key).startsWith(`${stageKey}:`))
          .map((row) => Number(String(row.action_key).split(':')[1]))
          .filter((value) => Number.isInteger(value));
        setCompleted(indexes);
        localStorage.setItem('nurj-actions-v2', JSON.stringify(indexes));
      });
    return () => {
      cancelled = true;
    };
  ...
  function toggleAction(index: number) {
    ...
    const next = isDone ? completed.filter((item) => item !== index) : [...completed, index];
    setCompleted(next);
    ...
    if (userId && supabase) {
      void supabase
        ...
        .then(({ error }) => {
          if (error) console.error('[nurj] action progress not saved', error.message);
        });
    }
  }
  ...
  return (
    <div className="screen-stack">
      ...
      <PlanExpiryNotice plan={profile.plan} expiresAt={profile.plan_expires_at} onUpgrade={() => onNavigate('upgrade')} />
...
function PromptStudio({
  ...
  const category = classifyBusiness(business);
  ...
  async function generate(event: FormEvent) {
    ...
    try {
      ...
      if (authenticated) {
        nextResult = await api.generate(payload);
      } else {
        // One real generation, server-side rate limited by IP. The old path
        // returned a fixed template behind a fake 900ms loader.
        try {
          nextResult = await api.generateGuest(payload);
        } catch (guestError) {
          const message = guestError instanceof Error ? guestError.message : '';
          ...
          // Network or model failure — fall back to the local scaffold rather
          // than showing a stranger nothing at all.
          nextResult = {
        ...
        <main className="studio-canvas panel">
          ...
          {step === 2 && (
            <form onSubmit={generate}>
              ...
              {wall === 'quota' && (
                <UpgradeWall
                  title="You have used today's five free prompts."
                  body="Builder removes the daily limit for 30 days. Your saved context and history stay exactly as they are."
                  actionLabel="See Builder"
                  onAction={onUpgrade}
                  onDismiss={() => setWall('none')}
                />
              )}
              {wall === 'guest' && (
                <UpgradeWall
                  title="That was your free preview."
                  body="Sign in with Google to keep generating — the free plan gives you five prompt architectures every day, and your history is saved."
    ...
    <div className="screen-stack">
      ...
      <section className="enhancer-grid">
        <article className="panel enhancer-input">
          ...
          {wall && (
            <UpgradeWall
              title="You have used today's three free enhancements."
              body="Builder removes the daily limit for 30 days."
              actionLabel="See Builder"
              onAction={onUpgrade}
...
function Guides({ stageKey, plan, onUpgrade }: { stageKey: StageKey; plan: PlanKey; onUpgrade: () => void }) {
  const [activeStage, setActiveStage] = useState(stageKey);
  const [open, setOpen] = useState(0);
  const [reading, setReading] = useState<GuideItem | null>(null);
  return (
    <div className="screen-stack">
      ...
      <AnimatePresence>{reading && <GuideReader item={reading} onClose={() => setReading(null)} />}</AnimatePresence>
    </div>
  );
}
  ...
  return (
    <motion.div className="guide-reader-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
      ...
      >
        <header>
          <div>
            <span className="eyebrow">PLAYBOOK · {item.minutes} MIN</span>
            <h2>{item.title}</h2>
            <p>{item.description}</p>
          </div>
          <button className="icon-button" onClick={onClose} aria-label="Close playbook"><ArrowLeft size={18} /></button>
        </header>
        <div className="guide-reader-body">
          {blocks.length === 0 && <p className="guide-empty">This playbook is being written. Check back shortly.</p>}
          {blocks.map((block, index) => {
            if (block.kind === 'callout') {
              return (
                <aside className="guide-callout" key={index}>
                  <Zap size={16} />
                  <div>{block.heading && <strong>{block.heading}</strong>}<p>{block.body}</p></div>
                </aside>
              );
            }
            if (block.kind === 'steps' || block.kind === 'list') {
              return (
                <section key={index}>
                  ...
                  {block.kind === 'steps' ? (
                    <ol className="guide-steps">{block.items?.map((entry) => <li key={entry}>{entry}</li>)}</ol>
                  ) : (
                    <ul className="guide-list">{block.items?.map((entry) => <li key={entry}><Check size={13} /> <span>{entry}</span></li>)}</ul>
                  )}
                </section>
              );
            }
...
function UpgradeWall({
  ...
  return (
    <motion.div className="upgrade-wall" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      <div className="upgrade-wall-icon"><Zap size={18} /></div>
      <div>
        <strong>{title}</strong>
        <p>{body}</p>
      </div>
      <div className="upgrade-wall-actions">
        <button className="button button-primary button-small" onClick={onAction}>{actionLabel} <ArrowRight size={14} /></button>
        <button className="button button-ghost button-small" onClick={onDismiss}>Not now</button>
      </div>
    </motion.div>
  );
}
...
function PaymentBanner({
  ...
  return (
    <div className={`payment-banner ${state}`}>
      {state === 'verifying' ? (
        <><ShieldCheck size={16} /><span>Confirming your payment with Paystack — this can take a moment.</span></>
      ) : (
        <>
          <ShieldCheck size={16} />
          <span>We could not confirm your payment yet. If money left your account it is safe — Paystack settlement can lag by a few minutes.</span>
          <button className="button button-small button-secondary" onClick={onRetry}>Check again</button>
          <button className="button button-small button-ghost" onClick={onDismiss}>Dismiss</button>
        </>
      )}
    </div>
  );
...
function PlanExpiryNotice({
  ...
  return (
    <div className="expiry-notice">
      ...
      <span>
        {daysLeft <= 0
          ? 'Your access has ended. Renew to remove the daily limits again.'
          : `Your ${plan === 'builder' ? 'Builder' : 'Operator'} access ends in ${daysLeft} day${daysLeft === 1 ? '' : 's'}.`}
      </span>
      <button className="button button-small button-secondary" onClick={onUpgrade}>Renew now</button>
    </div>
  );
}
...
function ShareButton({ text, title }: { text: string; title: string }) {
  ...
  return (
    <button className="copy-button" onClick={share}>
